...
#ifdef ENABLE_DECODE
#include <opentracedecode/libopentracedecode.h> /* First, so we avoid a _POSIX_C_SOURCE warning. */
#endif
...
{
	const QString stacktrace_outfile = temp_path + "/otv_stacktrace.txt";
	...
	otv::GlobalSettings settings;
	if (settings.value(otv::GlobalSettings::Key_Log_NotifyOfStacktrace).toBool()) {
		...
		msgbox.setText(QObject::tr("When %1 last crashed, it created a stack trace.\n" \
			...
			"the log. You may access it from the settings dialog.").arg(OTV_TITLE));
		...
		QObject::connect(cb, &QCheckBox::stateChanged, [](int state){
			otv::GlobalSettings settings;
			settings.setValue(otv::GlobalSettings::Key_Log_NotifyOfStacktrace,
				!state); });
...
void usage()
{
	fprintf(stdout,
		...
		"  -s, --settings                  Load OpenTraceView session setup from file\n"
...
{
	int ret = 0;
	shared_ptr<opentrace::Context> context;
	...
	otv::AndroidLogHandler::install_callbacks();
	otv::AndroidAssetReader asset_reader;
#endif
	...
	while (true) {
		...
		case 'l':
		{
			...
			context->set_log_level(opentrace::LogLevel::get(loglevel));

#ifdef ENABLE_DECODE
			otd_log_loglevel_set(loglevel);
#endif
	...
	qRegisterMetaType<otv::util::Timestamp>("util::Timestamp");
	qRegisterMetaType<SharedPtrToSegment>("SharedPtrToSegment");
	qRegisterMetaType<shared_ptr<otv::data::SignalBase>>("shared_ptr<SignalBase>");
	...
	context = opentrace::Context::create();
	otv::Session::otc_context = context;

#if ENABLE_STACKTRACE
	QString temp_path = QStandardPaths::standardLocations(
		QStandardPaths::TempLocation).at(0);
	stacktrace_filename = temp_path + "/otv_stacktrace.dmp";
	...
	do {
		...
		otv::DeviceManager device_manager(context, driver, do_scan);
		...
		if (show_version) {
			...
			otv::MainWindow w(device_manager);
			w.show();
