	...
	while (!interrupt_ && sample_count_) {
		...
		try {
			for (unsigned int i = 0; i < achannel_list.size(); i++) {
				shared_ptr<opentrace::Channel> achannel = (achannel_list.at(i))->channel();
				...
				auto analog = context->create_analog_packet(
					vector<shared_ptr<opentrace::Channel> >{achannel},
					(float *)adata, packet_len,
					opentrace::Quantity::VOLTAGE, opentrace::Unit::VOLT,
					vector<const opentrace::QuantityFlag *>());
