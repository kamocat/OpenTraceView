...
Decoder::Decoder(const otd_decoder *const dec, uint8_t stack_level) :
	otd_decoder_(dec),
	...
	decoder_inst_(nullptr)
{
	...
	i = 0;
	for (const GSList *rl = otd_decoder_->annotation_rows; rl; rl = rl->next) {
		const otd_decoder_annotation_row *const otd_row = (otd_decoder_annotation_row *)rl->data;
		assert(otd_row);
		rows_.emplace_back(i++, this, otd_row);

		// FIXME PV can crash from .at() if a PD's ann classes are defined incorrectly
		for (const GSList *cl = otd_row->ann_classes; cl; cl = cl->next)
...
const otd_decoder* Decoder::get_otd_decoder() const
{
	return otd_decoder_;
}
...
void Decoder::apply_all_options()
{
	if (decoder_inst_) {
		...
		otd_inst_option_set(decoder_inst_, opt_hash);
...
otd_decoder_inst* Decoder::create_decoder_inst(otd_session *session)
{
	GHashTable *const opt_hash = g_hash_table_new_full(g_str_hash,
		g_str_equal, g_free, (GDestroyNotify)g_variant_unref);
	...
	decoder_inst_ = otd_inst_new(session, otd_decoder_->id, opt_hash);
	g_hash_table_destroy(opt_hash);
	...
	otd_inst_channel_set_all(decoder_inst_, channels);

	otd_inst_initial_pins_set_all(decoder_inst_, init_pin_states);
...
{
	return (otd_decoder_->logic_output_channels != nullptr);
}
...
const vector<DecoderLogicOutputChannel> Decoder::logic_output_channels() const
{
	...
	for (GSList *l = otd_decoder_->logic_output_channels; l; l = l->next) {
		const otd_decoder_logic_output_channel* ch_data =
			(otd_decoder_logic_output_channel*)l->data;
