...
using opentrace::Context;
using opentrace::Driver;
	...
	context_(context)
{
	...
	if (!driver.empty()) {
		user_opts = otv::util::split_string(driver, ":");
	...
	if (!driver.empty()) {
		shared_ptr<opentrace::Driver> scan_drv;
...
const shared_ptr<opentrace::Context>& DeviceManager::context() const
{
	return context_;
}
	...
	shared_ptr<Driver> driver, map<const ConfigKey *, VariantBase> drvopts)
{
	...
	try {
		...
		for (shared_ptr<opentrace::HardwareDevice>& device : devices) {
	...
	shared_ptr<devices::Device> device)
{
	...
	if (otc_dev->vendor().length() > 0)
		result["vendor"] = otc_dev->vendor();
	if (otc_dev->model().length() > 0)
		result["model"] = otc_dev->model();
	if (otc_dev->version().length() > 0)
		result["version"] = otc_dev->version();
	if (otc_dev->serial_number().length() > 0)
		result["serial_num"] = otc_dev->serial_number();
	if (otc_dev->connection_id().length() > 0)
		result["connection_id"] = otc_dev->connection_id();
