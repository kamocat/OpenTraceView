...
using opentrace::Analog;
using opentrace::Channel;
using opentrace::ConfigKey;
using opentrace::DatafeedCallbackFunction;
using opentrace::Error;
using opentrace::InputFormat;
using opentrace::Logic;
using opentrace::Meta;
using opentrace::Packet;
using opentrace::Session;
...
using otv::data::SignalGroup;
using otv::util::Timestamp;
using otv::views::trace::Signal;
using otv::views::trace::AnalogSignal;
using otv::views::trace::LogicSignal;

namespace otv {

shared_ptr<opentrace::Context> Session::otc_context;
...
shared_ptr<opentrace::Session> Session::session() const
{
	if (!device_)
		return shared_ptr<opentrace::Session>();
...
void Session::set_main_bar(shared_ptr<otv::toolbars::MainBar> main_bar)
{
	main_bar_ = main_bar;
}

shared_ptr<otv::toolbars::MainBar> Session::main_bar() const
{
	return main_bar_;
}
...
void Session::set_device(shared_ptr<devices::Device> device)
{
	...
	for (auto& entry : cur_analog_segments_) {
		shared_ptr<opentrace::Channel>(entry.first).reset();
	...
	if (device_) {
		device_->session()->add_datafeed_callback([=]
			(shared_ptr<opentrace::Device> device, shared_ptr<Packet> packet) {
	...
	const string &format, const string &setup_file_name)
{
	...
	if (!format.empty()) {
		...
		auto user_opts = otv::util::split_string(format, ":");
...
void Session::load_file(QString file_name, QString setup_file_name,
	shared_ptr<opentrace::InputFormat> format, const map<string, Glib::VariantBase> &options)
{
...
void Session::start_capture(function<void (const QString)> error_handler)
{
	...
	const shared_ptr<opentrace::Device> otc_dev = device_->device();
	if (otc_dev) {
		const auto channels = otc_dev->channels();
...
double Session::get_samplerate() const
{
	...
	for (const shared_ptr<otv::data::SignalData>& d : all_signal_data_) {
		assert(d);
		const vector< shared_ptr<otv::data::Segment> > segments =
			d->segments();
		for (const shared_ptr<otv::data::Segment>& s : segments)
...
void Session::update_signals()
{
	...
	auto channels = otc_dev->channels();
	unsigned int logic_channel_count = count_if(
		channels.begin(), channels.end(),
		[] (shared_ptr<Channel> channel) {
			return channel->type() == opentrace::ChannelType::LOGIC; });
	...
	for (auto channel : otc_dev->channels()) {
	...
	if (signal_groups_.empty()) {
		for (auto& entry : otc_dev->channel_groups()) {
			...
			for (const shared_ptr<opentrace::Channel>& channel : group->channels()) {
...
shared_ptr<data::SignalBase> Session::signalbase_from_channel(
	shared_ptr<opentrace::Channel> channel) const
{
...
void Session::sample_thread_proc(function<void (const QString)> error_handler)
{
	...
	// Confirm that OTC_DF_END was received
	if (cur_logic_segment_)
		qDebug() << "WARNING: OTC_DF_END was not received.";
#endif
...
void Session::signal_new_segment()
{
	...
	if ((cur_logic_segment_ != nullptr) || !cur_analog_segments_.empty()) {
		...
		if (cur_logic_segment_) {
			...
			shared_ptr<opentrace::Channel> any_channel =
...
Gst::FlowReturn Session::on_gst_new_sample()
{
	...
	for (uint32_t block_id = 0; block_id < buf->n_memory(); block_id++) {
		...
		shared_ptr<opentrace::Packet> logic_packet =
			otc_context->create_logic_packet(mapinfo.get_data(), buf->get_size(), 1);

		try {
			feed_in_logic(dynamic_pointer_cast<opentrace::Logic>(logic_packet->payload()));
		} catch (bad_alloc&) {
			out_of_memory_ = true;
			device_->stop();
...
void Session::feed_in_logic(shared_ptr<opentrace::Logic> logic)
{
...
void Session::feed_in_analog(shared_ptr<opentrace::Analog> analog)
{
...
void Session::data_feed_in(shared_ptr<opentrace::Device> device,
...
#ifdef ENABLE_DECODE
void Session::on_new_decoders_selected(vector<const otd_decoder*> decoders)
{
	...
	if (signal)
		for (unsigned int i = 0; i < decoders.size(); i++) {
			const otd_decoder* d = decoders[i];
