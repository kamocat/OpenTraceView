...
const QString MainWindow::WindowTitle = tr("OpenTraceView");
	...
	Session &session)
{
	...
	if (type == views::ViewTypeTrace) {
		...
		if (!main_bar) {
			...
			otv::views::trace::StandardBar *standard_bar =
				new otv::views::trace::StandardBar(session, this, tv);
...
void MainWindow::setup_ui()
{
	...
	// Set the window icon
	QIcon icon;
	icon.addFile(QString(":/icons/opentraceview.png"));
	setWindowIcon(icon);
...
void MainWindow::update_acq_button(Session *session)
{
	...
	run_stop_button_->setText((state == otv::Session::Stopped) ?
		run_caption : tr("Stop"));
}
