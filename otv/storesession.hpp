...
#ifndef PULSEVIEW_OTV_STORESESSION_HPP
#define PULSEVIEW_OTV_STORESESSION_HPP
...
public:
	StoreSession(const string &file_name,
		const shared_ptr<opentrace::OutputFormat> &output_format,
...
private:
	void store_proc(vector< shared_ptr<data::SignalBase> > achannel_list,
		vector< shared_ptr<otv::data::AnalogSegment> > asegment_list,
		shared_ptr<otv::data::LogicSegment> lsegment);
...
private:
	...
	const shared_ptr<opentrace::OutputFormat> output_format_;
	const map<string, Glib::VariantBase> options_;
	const pair<uint64_t, uint64_t> sample_range_;
	const Session &session_;

	shared_ptr<opentrace::Output> output_;
...
#endif // PULSEVIEW_OTV_STORESESSION_HPP
