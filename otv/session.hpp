...
#ifndef PULSEVIEW_OTV_SESSION_HPP
#define PULSEVIEW_OTV_SESSION_HPP
...
public:
	...
	static shared_ptr<opentrace::Context> otc_context;
...
public:
	...
	shared_ptr<otv::toolbars::MainBar> main_bar() const;
	void set_main_bar(shared_ptr<otv::toolbars::MainBar> main_bar);
	...
	void load_file(QString file_name, QString setup_file_name = QString(),
		shared_ptr<opentrace::InputFormat> format = nullptr,
...
private:
	...
	void feed_in_header();
	void feed_in_meta(shared_ptr<opentrace::Meta> meta);
	void feed_in_trigger();
	void feed_in_frame_begin();
	void feed_in_frame_end();
	void feed_in_logic(shared_ptr<opentrace::Logic> logic);
	void feed_in_analog(shared_ptr<opentrace::Analog> analog);

	void data_feed_in(shared_ptr<opentrace::Device> device,
		shared_ptr<opentrace::Packet> packet);
...
#ifdef ENABLE_DECODE
	void on_new_decoders_selected(vector<const otd_decoder*> decoders);
#endif
...
private:
	...
	shared_ptr<otv::views::ViewBase> main_view_;

	shared_ptr<otv::toolbars::MainBar> main_bar_;
	...
	map< shared_ptr<opentrace::Channel>, shared_ptr<data::AnalogSegment> >
...
#endif // PULSEVIEW_OTV_SESSION_HPP
