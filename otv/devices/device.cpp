...
shared_ptr<opentrace::Session> Device::session() const
{
	return session_;
}

shared_ptr<opentrace::Device> Device::device() const
{
	return device_;
}

template uint64_t Device::read_config(const opentrace::ConfigKey*, const uint64_t);
