...
using opentrace::HardwareDevice;

namespace otv {
namespace devices {

HardwareDevice::HardwareDevice(const shared_ptr<opentrace::Context> &context,
	shared_ptr<opentrace::HardwareDevice> device) :
	context_(context),
	device_open_(false)
{
	device_ = device;
...
string HardwareDevice::full_name() const
{
	...
	return [&parts]() { QStringList list; for(const auto& part : parts) list << QString::fromStdString(part); return list.join(" ").toStdString(); }();
}

shared_ptr<opentrace::HardwareDevice> HardwareDevice::hardware_device() const
{
	return static_pointer_cast<opentrace::HardwareDevice>(device_);
}
