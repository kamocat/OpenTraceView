...
Logging::~Logging()
{
	...
	if (prev_sr_log_cb)
		otc_log_callback_set(prev_sr_log_cb, prev_sr_log_cb_data);
	prev_sr_log_cb = nullptr;
	prev_sr_log_cb_data = nullptr;
#ifdef ENABLE_DECODE
	if (prev_otd_log_cb)
		otd_log_callback_set(prev_otd_log_cb, prev_otd_log_cb_data);
	prev_otd_log_cb = nullptr;
	prev_otd_log_cb_data = nullptr;
#endif
...
void Logging::init()
{
	...
	otc_log_callback_set(log_sr, nullptr);
#ifdef ENABLE_DECODE
	otd_log_callback_get(&prev_otd_log_cb, &prev_otd_log_cb_data);
	otd_log_callback_set(log_srd, nullptr);
#endif
...
int Logging::get_log_level() const
{
	...
	return otc_log_loglevel_get();
}

void Logging::set_log_level(int level)
{
	otc_log_loglevel_set(level);
#ifdef ENABLE_DECODE
	otd_log_loglevel_set(level);
#endif
}
