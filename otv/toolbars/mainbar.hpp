...
#ifndef PULSEVIEW_OTV_TOOLBARS_MAINBAR_HPP
#define PULSEVIEW_OTV_TOOLBARS_MAINBAR_HPP
...
#include <otv/views/trace/standardbar.hpp>
#include <otv/widgets/devicetoolbutton.hpp>
...
Q_DECLARE_METATYPE(shared_ptr<opentrace::Device>)
...
class MainBar : public otv::views::trace::StandardBar
{
	Q_OBJECT
...
public:
	MainBar(Session &session, QWidget *parent,
		otv::views::trace::View *view);

	void update_device_list();

	void set_capture_state(otv::Session::capture_state state);
...
private Q_SLOTS:
	...
	void export_file(shared_ptr<opentrace::OutputFormat> format,
		bool selection_only = false, QString file_name = "");
	void import_file(shared_ptr<opentrace::InputFormat> format);
...
private:
	...
	otv::widgets::DeviceToolButton device_selector_;

	otv::widgets::PopupToolButton configure_button_;
	QAction *configure_button_action_;

	otv::widgets::PopupToolButton channels_button_;
	QAction *channels_button_action_;

	otv::widgets::SweepTimingWidget sample_count_;
	otv::widgets::SweepTimingWidget sample_rate_;
...
#endif // PULSEVIEW_OTV_TOOLBARS_MAINBAR_HPP
