...
#include <otv/devices/hardwaredevice.hpp>
#include <otv/devices/inputfile.hpp>
#include <otv/devices/sessionfile.hpp>
#include <otv/dialogs/connect.hpp>
#include <otv/dialogs/inputoutputoptions.hpp>
#include <otv/dialogs/storeprogress.hpp>
#include <otv/mainwindow.hpp>
#include <otv/popups/channels.hpp>
#include <otv/popups/deviceoptions.hpp>
#include <otv/util.hpp>
#include <otv/views/trace/view.hpp>
#include <otv/widgets/exportmenu.hpp>
#include <otv/widgets/importmenu.hpp>
#ifdef ENABLE_DECODE
...
using opentrace::InputFormat;
using opentrace::OutputFormat;
...
MainBar::MainBar(Session &session, QWidget *parent, otv::views::trace::View *view) :
	...
	add_math_signal_button_(new QToolButton())
{
	...
	connect(menu_file_export, SIGNAL(format_selected(shared_ptr<opentrace::OutputFormat>)),
		this, SLOT(export_file(shared_ptr<opentrace::OutputFormat>)));

	widgets::ImportMenu *menu_file_import = new widgets::ImportMenu(this,
		session.device_manager().context());
	menu_file_import->setTitle(tr("&Import"));
	connect(menu_file_import, SIGNAL(format_selected(shared_ptr<opentrace::InputFormat>)),
		this, SLOT(import_file(shared_ptr<opentrace::InputFormat>)));
	...
	connect(import_menu, SIGNAL(format_selected(shared_ptr<opentrace::InputFormat>)),
		this, SLOT(import_file(shared_ptr<opentrace::InputFormat>)));
	...
	connect(export_menu, SIGNAL(format_selected(shared_ptr<opentrace::OutputFormat>)),
		this, SLOT(export_file(shared_ptr<opentrace::OutputFormat>)));
	...
	set_capture_state(otv::Session::Stopped);
...
void MainBar::set_capture_state(otv::Session::capture_state state)
{
	bool ui_enabled = (state == otv::Session::Stopped) ? true : false;
...
void MainBar::update_sample_rate_selector()
{
	...
	const shared_ptr<opentrace::Device> otc_dev = device->device();

	sample_rate_.allow_user_entered_values(false);
	if (otc_dev->config_check(ConfigKey::EXTERNAL_CLOCK, Capability::GET)) {
		try {
			auto gvar = otc_dev->config_get(ConfigKey::EXTERNAL_CLOCK);
			if (gvar.gobj()) {
	...
	if (otc_dev->config_check(ConfigKey::SAMPLERATE, Capability::LIST)) {
		try {
			gvar_dict = otc_dev->config_list(ConfigKey::SAMPLERATE);
		} catch (Error& error) {
...
void MainBar::update_sample_count_selector()
{
	...
	if (otc_dev->config_check(ConfigKey::LIMIT_SAMPLES, Capability::LIST)) {
		try {
			auto gvar = otc_dev->config_list(ConfigKey::LIMIT_SAMPLES);
			if (gvar.gobj())
	...
	if (otc_dev->config_check(ConfigKey::LIMIT_SAMPLES, Capability::GET)) {
		auto gvar = otc_dev->config_get(ConfigKey::LIMIT_SAMPLES);
...
void MainBar::update_device_config_widgets()
{
	...
	DeviceOptions *const opts = new DeviceOptions(otc_dev, this);
	...
	if (otc_dev->config_check(ConfigKey::LIMIT_SAMPLES, Capability::SET))
...
void MainBar::commit_sample_rate()
{
	...
	try {
		otc_dev->config_set(ConfigKey::SAMPLERATE,
...
void MainBar::commit_sample_count()
{
	...
	if (sample_count_supported_) {
		try {
			otc_dev->config_set(ConfigKey::LIMIT_SAMPLES,
...
{
	using otv::dialogs::StoreProgress;
	...
	if (selection_only) {
		...
		const otv::util::Timestamp& start_time = trace_view->cursors()->first()->time();
		const otv::util::Timestamp& end_time = trace_view->cursors()->second()->time();

		const uint64_t start_sample = (uint64_t)max(
			0.0, start_time * samplerate);
		const uint64_t end_sample = (uint64_t)max(
			0.0, end_time * samplerate);
	...
	if (exts.empty())
		...
		filter += QString("(*.%1);;%2 (*)").arg(
			[&exts]() { QStringList list; for(const auto& ext : exts) list << QString::fromStdString(ext); return list.join(", *."); }(),
			tr("All Files"));
...
void MainBar::import_file(shared_ptr<InputFormat> format)
{
	...
	const QString filter_exts = exts.empty() ? "" : QString::fromStdString("%1 (%2)").arg(
		...
		QString::fromStdString("*.%1").arg([&exts]() { QStringList list; for(const auto& ext : exts) list << QString::fromStdString(ext); return list.join(" *."); }()));
...
{
	set_capture_state((otv::Session::capture_state)state);
}
...
void MainBar::on_config_changed()
{
	...
	// the user changed the OTC_CONF_EXTERNAL_CLOCK option. However,
...
void MainBar::on_actionSaveSetup_triggered()
{
	...
	const QString file_name = QFileDialog::getSaveFileName(
		this, tr("Save File"), dir, tr(
				"OpenTraceView Session Setups (*.pvs);;"
				"All Files (*)"));
...
bool MainBar::eventFilter(QObject *watched, QEvent *event)
{
	if (sample_count_supported_ && (watched == &sample_count_ ||
		...
		auto sec = otv::util::Timestamp(sample_count_.value()) / sample_rate_.value();
		QHelpEvent *help_event = static_cast<QHelpEvent*>(event);

		QString str = tr("Total sampling time: %1").arg(
			otv::util::format_time_si(sec, otv::util::SIPrefix::unspecified, 0, "s", false));
