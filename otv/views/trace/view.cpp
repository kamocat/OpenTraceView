...
#include "otv/views/trace/mathsignal.hpp"
#include "otv/views/trace/analogsignal.hpp"
#include "otv/views/trace/header.hpp"
#include "otv/views/trace/logicsignal.hpp"
#include "otv/views/trace/ruler.hpp"
#include "otv/views/trace/signal.hpp"
#include "otv/views/trace/tracegroup.hpp"
#include "otv/views/trace/triggermarker.hpp"
#include "otv/views/trace/viewport.hpp"

#ifdef ENABLE_DECODE
#include "otv/views/trace/decodetrace.hpp"
#endif

using otv::data::SignalBase;
using otv::data::SignalData;
using otv::data::Segment;
using otv::util::TimeUnit;
using otv::util::Timestamp;
...
void View::reset_view_state()
{
	...
	tick_period_ = 0;
	tick_prefix_ = otv::util::SIPrefix::yocto;
	tick_precision_ = 0;
...
void View::set_offset(const otv::util::Timestamp& offset, bool force_update)
{
...
void View::set_zero_position(const otv::util::Timestamp& position)
{
...
otv::util::Timestamp View::zero_offset() const
{
	return zero_offset_;
}
...
otv::util::SIPrefix View::tick_prefix() const
{
	return tick_prefix_;
}

void View::set_tick_prefix(otv::util::SIPrefix tick_prefix)
{
...
const otv::util::Timestamp& View::tick_period() const
{
	return tick_period_;
}
...
void View::set_tick_period(const otv::util::Timestamp& tick_period)
{
...
void View::set_time_unit(otv::util::TimeUnit time_unit)
{
...
{
	std::optional<Timestamp> left_time, right_time;
...
void View::set_cursors(otv::util::Timestamp& first, otv::util::Timestamp& second)
{
	assert(cursors_);
...
int64_t View::get_nearest_level_change(const QPoint &p)
{
	...
	for (entry_t &e : list) {
		...
		const int64_t x_offset = offset() / scale();
...
void View::calculate_tick_spacing()
{
	...
	otv::util::Timestamp tick_period = tick_period_;
	otv::util::SIPrefix tick_prefix = tick_prefix_;
	...
	do {
		...
		const otv::util::Timestamp order_decimal =
			pow(otv::util::Timestamp(10), order);
		...
		do {
			tp_with_margin = order_decimal *
				(ScaleUnits[unit++] + tp_margin);
		} while (tp_with_margin < min_period && unit < countof(ScaleUnits));

		minor_tick_count_ = (unit == 2) ? 4 : 5;
		tick_period = order_decimal * ScaleUnits[unit - 1];
		tick_prefix = static_cast<otv::util::SIPrefix>(
			(order - otv::util::exponent(otv::util::SIPrefix::yocto)) / 3);

		// Precision is the number of fractional digits required, not
		// taking the prefix into account (and it must never be negative)
		tick_precision = max((int)ceil(log10(1 / tick_period)), 0);

		tick_period_width = (tick_period / scale_);
...
void View::update_scroll()
{
	...
	int major_tick_distance = (int)(tick_period_ / scale_);
	...
	if (length < MaxScrollValue) {
		...
		hscrollbar->setSliderPosition(offset);
	} else {
...
TraceTreeItemOwner* View::find_prevalent_trace_group(
	const shared_ptr<opentrace::ChannelGroup> &group,
...
vector< shared_ptr<Trace> > View::extract_new_traces_for_channels(
	const vector< shared_ptr<opentrace::Channel> > &channels,
	const map<shared_ptr<data::SignalBase>, shared_ptr<Signal> > &signal_map,
...
bool View::eventFilter(QObject *object, QEvent *event)
{
	...
	if (type == QEvent::MouseMove) {
		...
		if (grabbed_widget_) {
			...
			otv::util::Timestamp mouse_time = offset_ + hover_point_.x() * scale_;
...
void View::update_view_range_metaobject() const
{
	...
	if (w > 0) {
		...
		const int64_t start_sample = (int64_t)(offset_ * samplerate);
		const int64_t end_sample = (int64_t)(offset_ * samplerate) +
...
void View::update_hover_point()
{
	...
	if (hover_point_.x() > 0) {
		...
		otv::util::Timestamp mouse_time = offset_ + hover_point_.x() * scale_;
		int64_t sample_num = (int64_t)(mouse_time * session_.get_samplerate());
...
{
	using opentrace::Channel;

	lock_guard<mutex> lock(signal_mutex_);

	vector< shared_ptr<Channel> > channels;
	shared_ptr<opentrace::Device> otc_dev;
	...
	if (!session_.device()) {
		...
		otc_dev = session_.device()->device();
		assert(otc_dev);
		channels = otc_dev->channels();
	}
	...
	// Make a look-up table of sigrok Channels to opentraceview Signals
	...
	if (otc_dev)
		for (auto& entry : otc_dev->channel_groups()) {
			const shared_ptr<opentrace::ChannelGroup> &group = entry.second;
	...
	copy_if(channels.begin(), channels.end(), back_inserter(logic_channels),
		[](const shared_ptr<Channel>& c) {
			return c->type() == opentrace::ChannelType::LOGIC; });
