...
using otv::data::LogicSegment;
using otv::data::SignalBase;
using otv::util::SIPrefix;
using otv::util::determine_value_prefix;
...
AnalogSignal::AnalogSignal(otv::Session &session, shared_ptr<data::SignalBase> base) :
	...
	autoranging_(true)
{
	...
	otv::data::Analog* analog_data =
		dynamic_cast<otv::data::Analog*>(base_->analog_data().get());
...
void AnalogSignal::paint_mid(QPainter &p, ViewItemPaintParams &pp)
{
	...
	if ((display_type_ == DisplayAnalog) || (display_type_ == DisplayBoth)) {
		...
		shared_ptr<otv::data::AnalogSegment> segment = get_analog_segment_to_paint();

		if (segment && (segment->get_sample_count() > 0)) {
			const double pixels_offset = pp.pixels_offset();
			const double samplerate = max(1.0, segment->samplerate());
			const otv::util::Timestamp& start_time = segment->start_time();
			const int64_t last_sample = (int64_t)segment->get_sample_count() - 1;
			const double samples_per_pixel = samplerate * pp.scale();
			const otv::util::Timestamp start = samplerate * (pp.offset() - start_time);
			const otv::util::Timestamp end = start + samples_per_pixel * pp.width();
...
void AnalogSignal::paint_trace(QPainter &p,
	const shared_ptr<otv::data::AnalogSegment> &segment,
...
shared_ptr<otv::data::AnalogSegment> AnalogSignal::get_analog_segment_to_paint() const
{
	shared_ptr<otv::data::AnalogSegment> segment;

	const deque< shared_ptr<otv::data::AnalogSegment> > &segments =
...
void AnalogSignal::perform_autoranging(bool keep_divs, bool force_update)
{
	...
	for (const shared_ptr<otv::data::AnalogSegment>& segment : segments) {
