...
Flag::Flag(View &view, const otv::util::Timestamp& time, const QString &text) :
...
QRectF Flag::label_rect(const QRectF &rect) const
{
	...
	if (!ref_item || (ref_item.get() == this)) {
		...
		const otv::util::Timestamp& delta = ref_item->delta(time_);
...
otv::widgets::Popup* Flag::create_popup(QWidget *parent)
{
	using otv::widgets::Popup;
