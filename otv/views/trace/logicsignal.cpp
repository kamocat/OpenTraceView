...
using opentrace::Trigger;
using opentrace::TriggerMatch;
using opentrace::TriggerMatchType;
...
LogicSignal::LogicSignal(otv::Session &session, shared_ptr<data::SignalBase> base) :
	Signal(session, base),
...
void LogicSignal::paint_mid(QPainter &p, ViewItemPaintParams &pp)
{
	...
	const uint64_t end_sample = min(max((int64_t)ceil(end),
		(int64_t)0), last_sample);
...
shared_ptr<otv::data::LogicSegment> LogicSignal::get_logic_segment_to_paint() const
{
	shared_ptr<otv::data::LogicSegment> segment;

	const deque< shared_ptr<otv::data::LogicSegment> > &segments =
...
const vector<int32_t> LogicSignal::get_trigger_types() const
{
	...
	const auto otc_dev = session_.device()->device();
	if (otc_dev->config_check(ConfigKey::TRIGGER_MATCH, Capability::LIST)) {
		const Glib::VariantContainerBase gvar =
			otc_dev->config_list(ConfigKey::TRIGGER_MATCH);
...
void LogicSignal::modify_trigger()
{
	...
	auto new_trigger = session_.device_manager().context()->create_trigger("opentraceview");
